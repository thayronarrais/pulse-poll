...
<div class="mb-6 flex items-center justify-between">
    <h1 class="text-3xl font-bold text-gray-800">{{ __('admin.live_form.create_title') }}</h1>
    <a href="{{ route('admin.live-sessions.index') }}" class="text-indigo-600 hover:text-indigo-900 font-medium">{{ __('messages.back') }}</a>
</div>

<form action="{{ route('admin.live-sessions.store') }}" method="POST" class="space-y-6 max-w-2xl">
    @csrf
    ...
    <div class="bg-white shadow-sm rounded-lg border border-gray-200 p-6">
        ...
        <div>
            ...
            <input type="text" name="title" id="title" required value="{{ old('title') }}" placeholder="{{ __('admin.live_form.title_placeholder') }}"
                   class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm p-2 border">
        </div>
        <div class="mt-4 bg-indigo-50 border border-indigo-100 rounded-md p-4 text-sm text-indigo-800">
            {!! __('admin.live_form.hint') !!}
        </div>
    </div>
    ...
    <div class="flex justify-end">
        <button type="submit" class="bg-green-600 hover:bg-green-700 text-white font-bold py-3 px-6 rounded-lg shadow-md text-lg">
            {{ __('admin.live_form.create_and_go') }}
        </button>
    </div>
</form>
@endsection
