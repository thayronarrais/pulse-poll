...
<div class="mb-6 flex items-center justify-between">
    <h1 class="text-3xl font-bold text-gray-800">{{ __('admin.survey_form.edit_title') }}</h1>
...
<form action="{{ route('admin.surveys.update', $survey) }}" method="POST" class="space-y-8" x-data="surveyForm()" @keydown.enter="$event.target.tagName !== 'TEXTAREA' ? $event.preventDefault() : null"  enctype="multipart/form-data">
    @csrf
    @method('PUT')
    ...
    <div class="bg-white shadow-sm rounded-lg border border-gray-200 p-6">
        ...
        <div class="space-y-4">
            <div>
                ...
                <input type="text" name="title" id="title" value="{{ old('title', $survey->title) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm p-2 border" required placeholder="{{ __('admin.survey_form.title_placeholder') }}">
            </div>

            <div>
                <label for="theme_color" class="block text-sm font-medium text-gray-700">{{ __('admin.survey_form.theme_color') }}</label>
                <div class="flex items-center mt-1">
                    <input type="color" name="theme_color" id="theme_color" value="{{ old('theme_color', $survey->theme_color ?? '#4f46e5') }}" class="h-10 w-14 rounded cursor-pointer border-0 p-0 mr-3">
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="3" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm p-2 border" placeholder="{{ __('admin.survey_form.description_placeholder') }}">{{ old('description', $survey->description) }}</textarea>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white shadow-sm rounded-lg border border-gray-200 p-6">
        ...
        <div class="space-y-6">
            <template x-for="(question, qIndex) in questions" :key="question.uniqueId">
                <div class="p-4 border border-gray-200 rounded-lg bg-gray-50 relative">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4 mt-6">
                        ...
                        <div class="md:col-span-2">
                            ...
                            <div x-show="question.image_path && !question.giphy_url" class="mt-2 text-sm text-gray-600">
                                {{ __('admin.survey_form.current_image') }} <img :src="question.image_path.startsWith('http') ? question.image_path : '/storage/' + question.image_path" class="mt-2 h-20 w-auto rounded border">
                                <label class="inline-flex items-center mt-2">
                                    <input type="checkbox" :name="`questions[${qIndex}][remove_image]`" value="1" class="rounded border-gray-300 text-red-600 shadow-sm focus:border-red-300 focus:ring focus:ring-red-200 focus:ring-opacity-50" @change="$event.target.checked ? question.image_path = null : null">
                                    <span class="ml-2 text-red-600 font-medium">{{ __('admin.survey_form.remove_existing_image') }}</span>
                                </label>
                            </div>

                            <!-- Hidden input for new Giphy URL -->
                            ...
                            <div x-show="question.giphy_url" style="display: none;" class="mt-3 relative inline-block">
                                <span class="block text-xs font-semibold text-gray-500 mb-1">{{ __('admin.survey_form.new_selected_gif') }}</span>
                    ...
                    <div class="mt-4 pl-4 border-l-2 border-indigo-200">
                        ...
                        <div class="space-y-2">
                            <template x-for="(option, oIndex) in question.options" :key="option.uniqueId">
    ...
    <div class="flex justify-end">
        <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 px-6 rounded-lg shadow-md transition duration-150 ease-in-out text-lg">
            {{ __('admin.survey_form.update_survey') }}
        </button>
    </div>
...
<script>
    function surveyForm() {
        const existingData = @json($survey->questions);
        
        let initialQuestions = existingData.map(q => {
            return {
                uniqueId: 'q_' + Date.now() + Math.random(),
                id: q.id,
                text: q.text,
                type: q.type,
                limit: q.limit,
                ...
                options: q.options.map(o => {
                    return {
                        uniqueId: 'o_' + Date.now() + Math.random(),
                        id: o.id,
                        text: o.text
                    };
                })
            ...
            initialQuestions = [
                {
                    ...
                    giphy_url: null,
                    options: [
                        { uniqueId: 'o_' + Date.now() + 1, id: '', text: '' },
                        { uniqueId: 'o_' + Date.now() + 2, id: '', text: '' }
                    ]
                }
            ];
        }
        ...
        return {
            ...
            selectGif(url) {
                ...
                // clear existing image so it's replaced
                this.questions[this.currentQuestionIndexForGiphy].image_path = null;
